...
public sealed class MainWindowViewModel : INotifyPropertyChanged
{
    ...
    private string _statusText = "Ready — add waveform groups to begin.";
    ...
    public IReadOnlyList<double> MahalanobisValues
    {
        get => _mahalanobisValues;
        private set { _mahalanobisValues = value; OnPropertyChanged(); }
    }

    public IReadOnlyList<ChartPointViewModel> MahalanobisPoints
    {
        get => _mahalanobisPoints;
        private set { _mahalanobisPoints = value; OnPropertyChanged(); }
    }
    ...
    public bool HasInspectionResult => _lastInspectionResult is not null;
    public bool IsModelTrained => _model is not null;
    public bool CanTrain => TrainCommand.CanExecute(null);
    public bool CanInspect => InspectCommand.CanExecute(null);
    public bool CanExport => ExportCommand.CanExecute(null);
    ...
    public double SampleIntervalSeconds
    {
        ...
        set { if (_sampleIntervalSeconds == value) return; _sampleIntervalSeconds = value; OnPropertyChanged(); }
    }
    ...
    public void AddGroup(IEnumerable<string> filePaths)
    {
        try
        {
            AddGroupModel(_application.LoadGroup(filePaths, SampleIntervalSeconds));
            SetStatus($"Added Group {ShortId(SelectedGroup!.Model)}: {SelectedGroup.RecordCount} waveform(s). {TrainingReadinessText()}");
        }
    ...
    public void AddGroupsFromRows(IEnumerable<string> filePaths)
    {
        try
        {
            foreach (var group in _application.LoadGroupsFromRows(filePaths, SampleIntervalSeconds))
                AddGroupModel(group);
            SetStatus($"Loaded groups from row-based CSV: {Groups.Count} group(s). {TrainingReadinessText()}");
        }
    ...
    public void ApplySettings(GroupDecisionPolicy policy, double confidence, double sampleIntervalSeconds)
    {
        ...
        SetStatus($"Settings applied: rule={policy.Rule}, confidence={confidence:G6}, dt={sampleIntervalSeconds:E3}s. Training is required again.");
        RaiseCommandStates();
    }
    ...
    private void ClearGroups()
    {
        ...
        OnPropertyChanged(nameof(IsModelTrained));
        SetStatus("Groups cleared. Add waveform groups to begin.");
        RaiseCommandStates();
    }
    ...
    private void ShowSelectedGroup()
    {
        ...
        SetStatus($"Selected Group {ShortId(group)}: {group.RecordCount} waveform(s). {TrainingReadinessText()}");
        OnPropertyChanged(nameof(HasSelectedGroup));
    }
    ...
    public void TrainModel()
    {
        ...
        {
            var normal = _groupModels.Where(g => !g.IsDefective).ToArray();
            var validation = _application.ValidateTraining(normal);
            if (!validation.IsValid)
            {
                _model = null;
                OnPropertyChanged(nameof(IsModelTrained));
                RaiseCommandStates();
                SetStatus($"Training validation failed: {validation.Issues.First(i => i.Code.StartsWith("ERROR_", StringComparison.Ordinal)).Message}");
                return;
            }

            _model = _application.Train(normal, Confidence);
            OnPropertyChanged(nameof(IsModelTrained));
            SetStatus($"Model trained: {normal.Length} normal groups, threshold={_model.Threshold:F6}. Select a group to inspect.");
            RaiseCommandStates();
        }
        catch (Exception ex)
        {
    ...
    private void Inspect()
    {
        ...
        try
        {
            ...
            if (!validation.IsValid)
            {
                ...
                OnPropertyChanged(nameof(IsModelTrained));
                RaiseCommandStates();
                SetStatus($"Inspection training validation failed: {validation.Issues.First(i => i.Code.StartsWith("ERROR_", StringComparison.Ordinal)).Message}");
                return;
            }
            ...
            OnPropertyChanged(nameof(HasInspectionResult));
            SetFeatures(result.Features);

            Deviations.Clear();
            foreach (var d in _application.AnalyzeDeviations(result.Features, _model))
                Deviations.Add(new FeatureDeviationViewModel(d.FeatureName, d.Value, d.Mean, d.StandardDeviation, d.ZScore, d.AbsoluteZScore, d.MahalanobisContribution));
        ...
        catch (Exception ex)
        {
            ...
            _lastSubgroupResults = Array.Empty<SubgroupInspectionResult>();
            OnPropertyChanged(nameof(IsModelTrained));
            OnPropertyChanged(nameof(HasInspectionResult));
            RaiseCommandStates();
    ...
    {
        if (SelectedGroup is null) return;
        if (SelectedGroup.IsDefective == value) return;
        ...
        OnPropertyChanged(nameof(IsModelTrained));
        SetStatus(value
            ? $"Group {ShortId(SelectedGroup.Model)} marked DEFECTIVE. Training model invalidated."
            : $"Group {ShortId(SelectedGroup.Model)} marked NORMAL. {TrainingReadinessText()}");
        RaiseCommandStates();
    }
    ...
    private void ClearInspectionResults()
    {
        _lastInspectionResult = null;
        _lastSubgroupResults = Array.Empty<SubgroupInspectionResult>();
        Deviations.Clear();
        Subgroups.Clear();
        MahalanobisValues = Array.Empty<double>();
        MahalanobisPoints = Array.Empty<ChartPointViewModel>();
        OnPropertyChanged(nameof(HasInspectionResult));
        RaiseCommandStates();
    }

    private string TrainingReadinessText()
    {
        var remaining = Math.Max(0, RequiredTrainingGroups - NormalGroupCount);
        return remaining == 0
            ? $"Training ready ({NormalGroupCount}/{RequiredTrainingGroups} normal groups)."
            : $"Training requires {remaining} more normal group(s) ({NormalGroupCount}/{RequiredTrainingGroups}).";
    }
