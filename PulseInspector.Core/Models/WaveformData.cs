...
public sealed class WaveformData
{
    ...
    {
        if (Samples.Length < 2) throw new InvalidOperationException("A waveform must contain at least two samples.");
        ...
        if (Samples.Any(x => !double.IsFinite(x))) throw new InvalidOperationException("Waveform contains a non-finite sample.");
    }
}
