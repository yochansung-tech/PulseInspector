...
public sealed record SubgroupDisplayRow(
    int Index,
    string SourceName,
...
public sealed class InspectionSelectionService
{
    ...
    public InspectionSelectionService(FeatureDeviationService? deviationService = null)
    {
        _deviationService = deviationService ?? new FeatureDeviationService();
    }

    public IReadOnlyList<SubgroupDisplayRow> CreateRows(IReadOnlyList<SubgroupInspectionResult> results)
    {
        return results
            .Select(r => new SubgroupDisplayRow(
                r.Index,
                r.SourceName,
                r.MahalanobisDistance,
                r.Threshold,
                r.IsDefect))
            .ToArray();
    }

    public SelectedSubgroupInspection Select(
        GroupData group,
        int recordIndex,
        ...
        InspectionModel model)
    {
        ...
        var result = results.FirstOrDefault(r => r.Index == recordIndex + 1);
        var deviations = _deviationService.Analyze(record.Features, model);
        return new SelectedSubgroupInspection(record, result, deviations);
    }
}
