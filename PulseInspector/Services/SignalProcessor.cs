...
public static class SignalProcessor
{
    public static double EstimateBaseline(IReadOnlyList<double> data, int count = 0)
    {
        ...
        // Do not assume that the pulse is at the beginning of the record.
        // The lower-amplitude portion is used so a positive pulse does not bias the baseline.
    ...
    public static double[] RemoveBaseline(IReadOnlyList<double> data, double baseline) =>
        data.Select(x => x - baseline).ToArray();
    ...
    public static double TrapezoidalIntegration(IReadOnlyList<double> data, double dt)
    {
        ...
        {
            // Charge is defined for the positive current component only.
    ...
    public static double EstimateNoise(IReadOnlyList<double> data, int count = 0)
    {
        ...
        count = count <= 0 ? Math.Max(2, data.Count / 5) : Math.Min(count, data.Count);
        var segment = data.OrderBy(x => x).Take(count).ToArray();
    ...
    {
        if (data.Count == 0 || dt <= 0 || low < 0 || high > 1 || low >= high) return 0;
        var peak = Peak(data).Value;
        if (peak <= 0) return 0;

        var lo = peak * low;
        var hi = peak * high;
        var loIndex = FirstCrossing(data, lo);
        var hiIndex = FirstCrossing(data, hi);
        return loIndex >= 0 && hiIndex >= loIndex ? (hiIndex - loIndex) * dt : 0;
    }

    public static double Fwhm(IReadOnlyList<double> data, double dt)
    {
        if (data.Count == 0 || dt <= 0) return 0;
        var peak = Peak(data).Value;
        if (peak <= 0) return 0;
