...
public sealed class MainForm : Form
{
    ...
    public MainForm()
    {
        ...
        addGroup.Click += (_, _) => AddGroupFromCsv(); file.DropDownItems.Add(addGroup);
        var addRowGroup = new ToolStripMenuItem("Add Group from CSV Rows...");
        addRowGroup.Click += (_, _) => AddGroupFromCsvRows(); file.DropDownItems.Add(addRowGroup);
        var clear = new ToolStripMenuItem("Clear Groups");
        clear.Click += (_, _) => ClearGroups(); file.DropDownItems.Add(clear); menu.Items.Add(file);

        var training = new ToolStripMenuItem("Training");
        var train = new ToolStripMenuItem("Train Normal Groups");
        train.Click += (_, _) => TrainModel(); training.DropDownItems.Add(train);
        var inspect = new ToolStripMenuItem("Inspect Selected Group");
        inspect.Click += (_, _) => InspectSelectedGroup(); training.DropDownItems.Add(inspect); menu.Items.Add(training);

        var settings = new ToolStripMenuItem("Settings");
        settings.Click += (_, _) => new SettingsForm().ShowDialog(this); menu.Items.Add(settings);
        var about = new ToolStripMenuItem("About");
        about.Click += (_, _) => new AboutForm().ShowDialog(this); menu.Items.Add(about);
        ...
        _subgroupList.Columns.Add("#", 50); _subgroupList.Columns.Add("Source", 180);
        _subgroupList.Columns.Add("Mahalanobis", 110); _subgroupList.Columns.Add("Threshold", 110); _subgroupList.Columns.Add("Result", 90);

        var groupPanel = new Panel { Dock = DockStyle.Left, Width = 300, Padding = new Padding(8) };
        groupPanel.Controls.Add(_groupList); groupPanel.Controls.Add(_defective);
        ...
        subgroupPanel.Controls.Add(_subgroupList); subgroupPanel.Controls.Add(_subgroupHeader);

        var analysisSplit = new SplitContainer { Dock = DockStyle.Fill, Orientation = Orientation.Horizontal, SplitterDistance = 400 };
        var waveformFeatureSplit = new SplitContainer { Dock = DockStyle.Fill, Orientation = Orientation.Vertical, SplitterDistance = 850 };
        waveformFeatureSplit.Panel1.Controls.Add(_waveform);
        waveformFeatureSplit.Panel2.Controls.Add(_features);
        analysisSplit.Panel1.Controls.Add(waveformFeatureSplit);
        analysisSplit.Panel2.Controls.Add(_deviations);
        center.Controls.Add(analysisSplit); center.Controls.Add(subgroupPanel);
        Controls.Add(center); Controls.Add(groupPanel);

        var status = new Panel { Dock = DockStyle.Bottom, Height = 34 };
        status.Controls.Add(_status); Controls.Add(status);
    }

    private void AddGroupFromCsv()
    {
        using var dialog = new OpenFileDialog { Filter = "CSV files (*.csv)|*.csv|All files (*.*)|*.*", Multiselect = true, Title = "Select waveforms belonging to one group" };
        ...
        try
        {
            foreach (var file in dialog.FileNames)
            {
                ...
                group.AddWaveform(data.Samples, features, data.SourceName, data.SampleIntervalSeconds, data.HasExplicitTimeAxis);
            }
            AddGroupToUi(group);
        }
    ...
    {
        using var dialog = new OpenFileDialog { Filter = "CSV files (*.csv)|*.csv|All files (*.*)|*.*", Title = "Select a CSV where each row is one subgroup" };
        if (dialog.ShowDialog(this) != DialogResult.OK) return;
        try
        {
    ...
    {
        _groups.Add(group); _groupList.Items.Add(CreateGroupLabel(group)); _groupList.SelectedIndex = _groups.Count - 1;
        _model = null; _deviations.SetResults(Array.Empty<FeatureDeviation>());
    ...
    private void InspectSelectedGroup()
    {
        ...
        if (index < 0 || index >= _groups.Count) { MessageBox.Show(this, "Select a group first."); return; }
        ...
        {
            MessageBox.Show(this, $"Inspection requires at least {required} normal training groups excluding the selected group.", "Training data insufficient", MessageBoxButtons.OK, MessageBoxIcon.Warning); return;
        }
        try
        {
            ...
            _deviations.SetResults(_deviationService.Analyze(result.Features, _model));
            _status.SetState(!result.IsDefect, $"Group {ShortId(selected)}: {(result.IsDefect ? "DEFECT" : "NORMAL")} | MD={result.MahalanobisDistance:F6}, Threshold={result.Threshold:F6} | Subgroups={_subgroupResults.Count(r => r.IsDefect)}/{_subgroupResults.Count} defective");
        }
    ...
    {
        _subgroupResults.Clear(); _subgroupList.Items.Clear(); if (_model is null) return;
        _subgroupResults.AddRange(_subgroupService.Inspect(group, _model));
        foreach (var result in _subgroupResults)
        {
            var item = new ListViewItem(result.Index.ToString()); item.SubItems.Add(result.SourceName);
            item.SubItems.Add(result.MahalanobisDistance.ToString("F6")); item.SubItems.Add(result.Threshold.ToString("F6")); item.SubItems.Add(result.IsDefect ? "DEFECT" : "NORMAL");
    ...
    {
        var index = _groupList.SelectedIndex; if (index < 0 || index >= _groups.Count) return;
        var group = _groups[index]; var waveform = group.MeanWaveform(); if (waveform is not null) _waveform.SetData(waveform);
        var features = group.MeanFeatures(); if (features is not null) _features.SetFeatures(features);
        _deviations.SetResults(Array.Empty<FeatureDeviation>());
        _updatingDefective = true; _defective.Checked = group.IsDefective; _updatingDefective = false;
        _subgroupResults.Clear(); _subgroupList.Items.Clear(); _status.SetState(true, $"Selected Group {ShortId(group)}: {group.SampleCount} waveform(s)");
    }

    private void ShowSelectedSubgroup()
    {
        if (_groupList.SelectedIndex < 0 || _subgroupList.SelectedIndices.Count == 0) return;
        var group = _groups[_groupList.SelectedIndex]; var row = _subgroupList.SelectedItems[0];
        if (row.Tag is not int recordIndex || recordIndex < 0 || recordIndex >= group.Records.Count) return;
        var record = group.Records[recordIndex]; _waveform.SetData(record.Samples); _features.SetFeatures(record.Features);
        if (_model is not null) _deviations.SetResults(_deviationService.Analyze(record.Features, _model));
        var result = _subgroupResults.FirstOrDefault(r => r.Index == recordIndex + 1);
        _status.SetState(result is null || !result.IsDefect, result is null ? $"Subgroup {recordIndex + 1}: {record.SourceName}" : $"Subgroup {result.Index}: {(result.IsDefect ? "DEFECT" : "NORMAL")} | MD={result.MahalanobisDistance:F6}, Threshold={result.Threshold:F6}");
    }

    private void UpdateSelectedGroupLabel()
    {
        if (_updatingDefective) return; var index = _groupList.SelectedIndex; if (index < 0 || index >= _groups.Count) return;
        _groups[index].IsDefective = _defective.Checked; _groupList.Items[index] = CreateGroupLabel(_groups[index]); _groupList.SelectedIndex = index;
        _model = null; _subgroupResults.Clear(); _subgroupList.Items.Clear(); _deviations.SetResults(Array.Empty<FeatureDeviation>());
    }

    private void ClearGroups()
    {
        _groups.Clear(); _groupList.Items.Clear(); _subgroupResults.Clear(); _subgroupList.Items.Clear(); _model = null;
        _waveform.SetData(Array.Empty<double>()); _deviations.SetResults(Array.Empty<FeatureDeviation>()); _status.SetState(false, "Groups cleared");
    }
